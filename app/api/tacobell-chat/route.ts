...
export function detectUpsellAttempt(aiResponse: string): boolean {
  ...
  return lower.includes("would you like") || lower.includes("add") || lower.includes("try") ||
    ...
    lower.includes("upgrade") || lower.includes("combo") || lower.includes("wash it down") ||
    lower.includes("top it off") || lower.includes("something to drink") || lower.includes("baja blast") ||
    lower.includes("cinnamon twists") || lower.includes("nachos") || lower.includes("wash it down with");
}

export function detectUpsellAcceptance(userMessage: string): boolean {
  const lower = userMessage.toLowerCase();
  const acceptPhrases = ["yes", "yeah", "sure", "okay", "add", "get", "i'll take", "yes please", "do it", "go ahead", "yep", "yup", "definitely"];
